calculate_all_probs <- function(z){
...
#  colnames(probs) <- colnames(z)
  rownames(wneg) <- rownames(w0) <- rownames(wpos) <- rownames(z)
  colnames(wneg) <- colnames(w0) <- colnames(wpos) <- colnames(z)
  
#  probs <- probs[,order(colnames(probs))]
  return(list(pneg=wneg, p0=w0, ppos=wpos))
}
